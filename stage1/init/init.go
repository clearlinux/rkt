...
func getNspawnArgsEnv(p *Pod, debug bool) ([]string, []string, error) {
	...
	// before v220, we should instruct to keep extra rkt fd.
	if p.Stage1Options.flavor == "src" &&
		(p.Stage1Options.systemdversion == "v215" ||
			p.Stage1Options.systemdversion == "v219") {
